    ...
    public class UI_TextComplex {
        ...
        public UI_TextComplex(Transform parent, Vector2 anchoredPosition, int fontSize, char iconChar, string text,
            ...
            while (tmp.IndexOf(iconChar) != -1) {
                ...
                if (untilTmp.Trim() != "") {
                    var uiText = UtilsClass.DrawTextUI(untilTmp, transform, new Vector2(textPosition, 0), fontSize,
                        font);
            ...
            if (tmp.Trim() != "") UtilsClass.DrawTextUI(tmp, transform, new Vector2(textPosition, 0), fontSize, font);
        }
        ...
        public float GetTotalWidth() {
            ...
            foreach (Transform trans in transform) {
                ...
                if (image != null) textPosition += image.GetComponent<RectTransform>().sizeDelta.x;
            }
        ...
        public void AddTextOutline(Color color, float size) {
            foreach (Transform textComplexTrans in transform)
                if (textComplexTrans.GetComponent<Text>() != null) {
                    var outline = textComplexTrans.gameObject.AddComponent<Outline>();
