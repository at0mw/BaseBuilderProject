...
namespace CodeMonkey.Utils {
    ...
    public static class UtilsClass {
        ...
        public static Vector3 GetClosestPointAlongAxis(Vector3 pointA, Vector3 testPoint, Vector3 axis) {
            ...
            var toPoint = testPoint - pointA;
            var toPointDistance = toPoint.magnitude;

            // Calculate theta (angle)
            var theta = Vector3.Angle(axis, toPoint);

            // Using CAH rule (cosine, adjacent, hypotenuse) to find the (adjacent) side length
            var pointIntersectionDistance = Mathf.Cos(theta * Mathf.Deg2Rad) * toPointDistance;

            // Travelling the calculated distance in the direction of the target
            var intersectionPoint = pointA + axis * pointIntersectionDistance;
